...
class ImageLinks_Admin {
	...
	function view_edit($views) {
		$views['import'] = __('<div class="updated"><p>Note: This is the lite version of the ImageLinks WordPress plugin. You can create and use only one item, also the final image will have a watermark.<br/><a href="https://codecanyon.net/item/imagelinks-interactive-image-builder-for-wordpress/16654231?ref=avirtum" target="_blank">Click here to purchase pro version</a></p></div>', $this->plugin_name);
		return $views;
	}
	 ...
	 * @since 1.3.0
	 */
	function insert_post_data($maybe_empty, $postarr) {
		if($postarr['post_type'] == $this->post_type) {
			global $wpdb;
			$rows = $wpdb->get_results("SELECT ID as post_id, post_status FROM " . $wpdb->posts . " WHERE post_type = 'imgl_item' AND post_status = 'publish'", OBJECT);
			$rowcount = count($rows);
			
			$flag = false;
			if($rowcount == 1) {
				foreach ($rows as $row) {
					if($postarr['ID'] == $row->post_id) {
						$flag = true;
						break;
					}
				}
			...
			if($rowcount > 0 && $postarr['post_status'] == 'publish' && !$flag) {
	...
	function redirect_post_location_filter($location) {
		remove_filter('redirect_post_location', array($this, 'redirect_post_location_filter'), 99);
	...
	public function post_updated_messages( $messages ) {
		...
		$messages[$this->post_type][99] = __( 'Publish not allowed. You can create and use only one item.<br/><a href="https://codecanyon.net/item/imagelinks-interactive-image-builder-for-wordpress/16654231?ref=avirtum" target="_blank">Click here to purchase pro version</a>', $this->plugin_name);
